...
// Convert to numbers and add sums
	...
	var scores = {totalScore: totalScore, homeScore: home, travelScore: travel, foodScore: food, shoppingScore: shopping}; 
	return scores;
};
	...
	$.ajax({
		...
		success: function(result){
			for (let i = 0 ; i < result.length ; i++){
				...
				$("#leaderboard").append("<i class='fa fa-user'></i><li>" + result[i]['display_name'] + " : " + result[i]['total_score'] + "</li>")
			};
		...
		error: function(result){
			...
			$("#leaderboard-error").html(result.status +''+ result.statusText +''+result.responseText);
		}
	});
};
...
var submitNewTotal = function(display_name, totalScore){
	let the_serialized_data = $("#calculator").serialize() + "&userid=" + localStorage.usertoken + "&display_name="+ display_name + "&totalScore=" + totalScore;
	...
	$.ajax({
		...
		success: function(result){
			...
			$("#message").html("You logged a waste score of " + totalScore + ". Check the breakdown on the dashboard!");
		},
		error: function(result){
			//console.log(result);
			$('#message').addClass('alert alert-danger');
			$('#message').html(result.status +''+ result.statusText +''+result.responseText );
		}
	});
};
...
var sendTip = function(){
	...
	let the_serialized_data = $("#tip-form").serialize() + "&userid=" + localStorage.usertoken;
	console.log(the_serialized_data);
